Browse.init = function (options) {
  ...
  this.progFx = new Fx.Elements($$('#tagPics', '#tagContainer', '#tagProgress'), {
    'link': 'chain',
    'onComplete': function () {
      this.progCount += this.progCount == 3 ? - 4 : 1;
    }.bind(this)
  });

  // Make tagProgress visible but with zero opacity (allows us to calculate height).
  $('tagProgress').setStyles({
    'opacity': 0,
    'visibility': 'visible'
  });
};

Browse.tagClick = function (l, type) {
  // If we are currently in animation then clicking does nothing.
  if (this.progCount > - 1) return;

  if (type == "tag") ident = /tag-link-(\d+)/.exec(l.get('class'))[1];
  else if (type == "cat") ident = /cat-item-(\d+)/.exec(l.getParent().get('class'))[1];
  else ident = l.innerHTML;
  ...
  var cur = $$('a.current');
  if (cur.length > 0) cur[0].removeClass('current');
  l.addClass('current');
...
Browse.tagRefresh = function (data) {
  // Create asset manager to grab all images from the server.
  srcArray = new Array();
  for (i = 0; i < data.length; i++)
  srcArray[i] = data[i].image_uri;
  this.preload = new Asset.images(srcArray, {
    onComplete: this.preloadFinish.bind(this)
  });
